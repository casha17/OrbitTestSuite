...
module Utilities = 
    ...
    let compareFileMetadata (x: metadata) (y: metadata) =
        (x.id = y.id) && (x.name = y.name) && (x.parentId = y.parentId) && (x.timestamp = y.timestamp) && (x.version = y.version) && (x.versionChanged = y.versionChanged)
